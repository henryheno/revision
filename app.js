...
let currentIndex = 0;
let currentMode = 'qcm'; // 'qcm' ou 'card'

// État des réponses de l'utilisateur : { [questionIndex]: { selectedIndex, isCorrect } }
const userAnswers = {};
let score = 0;
let answeredCount = 0;
...
// Rendu de la question selon le mode actif
...
function renderQcmView(item) {
    ...
    qCard.textContent = item.question;
    qcmContainer.appendChild(qCard);

    // Conteneur des propositions (A, B, C, D)
    ...
    // Si déjà répondu, afficher l'explication
    if (isAnswered) {
...
function handleOptionSelect(item, selectedIdx, container, optionsList) {
    if (userAnswers[currentIndex]) return; // Déjà répondu
    ...
    // Désactiver tous les boutons d'options et colorer
...
// Basculer entre Mode QCM et Mode Carte
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'ArrowRight') {
        ...
        // Touches 1-4 ou A-D pour répondre directement au clavier
